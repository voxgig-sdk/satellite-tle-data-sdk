...
local TleEntity = {}
TleEntity.__index = TleEntity


function TleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TleEntity)
  self._name = "tle"
  self._client = client
...
function TleEntity:get_name()
  return self._name
end


function TleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TleEntity.new(self._client, opts)
end
...
function TleEntity:mark_deleted()
  self._deleted = true
end


function TleEntity:deleted()
  return true == self._deleted
end


function TleEntity:data_set(args)
  if args ~= nil then
...
function TleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TleEntity:match_set(args)
  if args ~= nil then
...
function TleEntity:stream(action, args, callopts)
...
---@param reqmatch TleLoadMatch
---@param ctrl? table
---@return Tle
---@return string? err
function TleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch TleListMatch
---@param ctrl? table
---@return Tle[]
---@return string? err
function TleEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function TleEntity:_run_op(ctx, post_done)
  local utility = self._utility
