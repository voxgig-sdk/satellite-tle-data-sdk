...
describe("TleDirect", function()
  it("should direct-list-tle", function()
    local setup = tle_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-tle", setup.live and "live" or "unit")
  ...
  it("should direct-load-tle", function()
    local setup = tle_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-tle", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function tle_direct_setup(mockres)
