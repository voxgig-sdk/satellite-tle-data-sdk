# satellite-tle-data-cli

boru-driven command-line client **and** interactive REPL for the SatelliteTleData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/satellite-tle-data-cli)
make build

# 2. See usage (words, entities, env vars)
./satellite-tle-data-cli --help

# 3. Provide credentials once, via the environment
export SATELLITE_TLE_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./satellite-tle-data-cli list tle
./satellite-tle-data-cli load 1 tle            # {id:1} shorthand
./satellite-tle-data-cli load '{id:1}' tle       # explicit match map

# 5. Override the API base URL for a single call
SATELLITE_TLE_DATA_BASE=https://api.example.com ./satellite-tle-data-cli list tle

# 6. No arguments -> interactive REPL
./satellite-tle-data-cli
satellite-tle-data> list tle
satellite-tle-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/satellite-tle-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/satellite-tle-data-cli list tle
   ```
...
```sh
./satellite-tle-data-cli load 1 tle          # scalar shorthand for {id:1}
./satellite-tle-data-cli load '{id:1}' tle     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `satellite-tle-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./satellite-tle-data-cli
satellite-tle-data> list tle
satellite-tle-data> /help
satellite-tle-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `tle`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/satellite-tle-data-cli`. |
...
(`./satellite-tle-data-cli <expr>`) and interactively (the REPL), and expressions compose the
