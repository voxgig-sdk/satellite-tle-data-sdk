# SatelliteTleData SDK test runner
...
class SatelliteTleDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SatelliteTleDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SatelliteTleDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SatelliteTleDataTestRunner.getenv("SATELLITE_TLE_DATA_TEST_LIVE")
        override = SatelliteTleDataTestRunner.getenv("SATELLITE_TLE_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SatelliteTleDataTestRunner.getenv(key)
        ...
        explain = SatelliteTleDataTestRunner.getenv("SATELLITE_TLE_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SATELLITE_TLE_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SatelliteTleDataTestRunner._test_control is not None:
            return SatelliteTleDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SatelliteTleDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SatelliteTleDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SatelliteTleDataTestRunner.load_test_control()
...
def load_env_local():
    SatelliteTleDataTestRunner.load_env_local()


def env_override(m):
    return SatelliteTleDataTestRunner.env_override(m)


def entity_data(v):
    return SatelliteTleDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SatelliteTleDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SatelliteTleDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SatelliteTleDataTestRunner.load_test_control()


def live_client_options():
    return SatelliteTleDataTestRunner.live_client_options()


def live_delay_ms():
    return SatelliteTleDataTestRunner.live_delay_ms()
