...
class TestTleEntity:
    ...
    def test_should_stream(self):
        ...
        base = SatelliteTleDataSDK.test(seed, None)
        seen = list(base.Tle(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from satellitetledata_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = SatelliteTleDataSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Tle(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _tle_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "tle." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SATELLITE_TLE_DATA_TEST_TLE_ENTID JSON to run live")
        client = setup["client"]
        ...
        tle_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(tle_ref01_data_dt0_loaded))
        assert tle_ref01_data_dt0_load_result is not None
        assert tle_ref01_data_dt0_load_result["id"] == tle_ref01_data["id"]



def _tle_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/tle/TleTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SATELLITE_TLE_DATA_TEST_TLE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("SATELLITE_TLE_DATA_TEST_LIVE") == "TRUE":
        ...
        client = SatelliteTleDataSDK(helpers.to_map(merged_opts))

    _live = env.get("SATELLITE_TLE_DATA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("SATELLITE_TLE_DATA_TEST_EXPLAIN") == "TRUE",
        "live": _live,
