...
require_relative 'SatelliteTleData_types'
...
class SatelliteTleDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = SatelliteTleDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SatelliteTleDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SatelliteTleDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SatelliteTleDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SatelliteTleDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SatelliteTleDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SatelliteTleDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue SatelliteTleDataError => err
    ...
    if fetched.is_a?(Hash)
      status = SatelliteTleDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SatelliteTleDataError.new(
        "graphql_error", "SatelliteTleDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SatelliteTleDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
