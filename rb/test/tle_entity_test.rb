...
class TleEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = SatelliteTleDataSDK.test(seed, nil)
    seen = base.Tle(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Tle(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SATELLITE_TLE_DATA_TEST_TLE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    tle_ref01_data_dt0_load_result = Helpers.to_map(tle_ref01_data_dt0_loaded.respond_to?(:data_get) ? tle_ref01_data_dt0_loaded.data_get : tle_ref01_data_dt0_loaded)
...
def tle_basic_setup(extra)
  ...
  entid_env_raw = ENV["SATELLITE_TLE_DATA_TEST_TLE_ENTID"]
