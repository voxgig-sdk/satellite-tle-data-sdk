...
module SatelliteTleDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SatelliteTleDataResponse.new({})
    ...
    if fetch_err
      ...
      response = SatelliteTleDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SatelliteTleDataResponse.new(fetched)
    else
