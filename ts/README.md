# SatelliteTleData TypeScript SDK



The TypeScript SDK for the SatelliteTleData API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Tle()` — each with a small set of operations (`list`, `load`)
...
```ts
try {
  const tle = await client.Tle().load({ id: 1 })
  console.log(tle)
} catch (err) {
...
| `tester(testopts?, sdkopts?)` | `SatelliteTleDataSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `SatelliteTleDataSDK.test(testopts?, sdkopts?)` | `SatelliteTleDataSDK` | Create a test-mode client. |
...
| `client` | `client(): SatelliteTleDataSDK` | Return the parent SDK client. |
...
satellite-tle-data/
├── src/
│   ├── SatelliteTleDataSDK.ts        # Main SDK class
