...
//     (\`new SatelliteTleDataSDK(...)\` / \`SatelliteTleDataSDK.test(...)\` become
//     \`SatelliteTleDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SatelliteTleDataSDK'
...
const TEST_SEED = {"entity":{"tle":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SatelliteTleDataSDK(...)\` and any
// \`SatelliteTleDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SatelliteTleDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SatelliteTleDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
