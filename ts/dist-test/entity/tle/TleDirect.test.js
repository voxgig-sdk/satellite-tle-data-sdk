...
(0, node_test_1.describe)('TleDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.SatelliteTleDataSDK({
    ...
    (0, node_test_1.test)('direct-load-tle', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-tle', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-tle', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-tle', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.SATELLITE_TLE_DATA_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.SatelliteTleDataSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['SATELLITE_TLE_DATA_TEST_TLE_ENTID'];
    ...
    const client = new __1.SatelliteTleDataSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=TleDirect.test.js.map