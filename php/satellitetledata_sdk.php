...
class SatelliteTleDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = SatelliteTleDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = SatelliteTleDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!SatelliteTleDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, SatelliteTleDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return SatelliteTleDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = SatelliteTleDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = SatelliteTleDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = SatelliteTleDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new SatelliteTleDataError($op . "_allow",
                "SatelliteTleDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = SatelliteTleDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new SatelliteTleDataError("graphql_error",
                "SatelliteTleDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Tle($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_tle === null) {
                $this->_tle = new TleEntity($this, null);
            }
            return $this->_tle;
        }
        return new TleEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new SatelliteTleDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
