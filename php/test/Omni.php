...
//    only - a typed SatelliteTleDataContext there would make every ctx
...
final class SatelliteTleDataOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SatelliteTleDataContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SatelliteTleDataSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SatelliteTleDataResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SatelliteTleDataError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SatelliteTleDataResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SatelliteTleDataError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SatelliteTleDataError(is_string($code) ? $code : '', $msg);
    }
