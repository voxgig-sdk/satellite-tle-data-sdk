...
// SatelliteTleData SDK feature test
//
...
class FtCtrl extends SatelliteTleDataControl
{
...
class FtClient
{
    ...
    private SatelliteTleDataUtility $utility;

    public function __construct(SatelliteTleDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): SatelliteTleDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SatelliteTleDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SatelliteTleDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SatelliteTleDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SatelliteTleDataConfig::make_feature($name);
    ...
    private static function buildUrl(SatelliteTleDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SatelliteTleDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SatelliteTleDataResponse($fetched);
        }
    ...
    private function fail(SatelliteTleDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SatelliteTleDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
