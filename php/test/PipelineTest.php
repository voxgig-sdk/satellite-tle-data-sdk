...
// SatelliteTleData SDK pipeline test
//
...
{
    private static function utility(): SatelliteTleDataUtility
    {
        return new SatelliteTleDataUtility();
    }
    ...
    private static function ctx(array $over = []): SatelliteTleDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SatelliteTleDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SatelliteTleDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SatelliteTleDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SatelliteTleDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SatelliteTleDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SatelliteTleDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SatelliteTleDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SatelliteTleDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SatelliteTleDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SatelliteTleDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SatelliteTleDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SatelliteTleDataResponse([]),
            'result' => new SatelliteTleDataResult([]),
        ]);
        [, $err] = SatelliteTleDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new SatelliteTleDataSpec([]),
            'response' => null,
            'result' => new SatelliteTleDataResult([]),
        ]);
        [, $err] = SatelliteTleDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SatelliteTleDataSpec(['step' => 's']),
            'response' => new SatelliteTleDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SatelliteTleDataResult([]),
        ]);
        [, $err] = SatelliteTleDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SatelliteTleDataSpec(['step' => 's']),
            'response' => new SatelliteTleDataResponse(self::resp(200, ['v' => 1])),
            'result' => new SatelliteTleDataResult([]),
        ]);
        [, $err] = SatelliteTleDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SatelliteTleDataSpec(['step' => 's']),
            'response' => new SatelliteTleDataResponse(self::resp(200, ['v' => 2])),
            'result' => new SatelliteTleDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SatelliteTleDataMakeResponse::call($ctx);
    ...
    {
        $preset = new SatelliteTleDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SatelliteTleDataSpec([]),
            'response' => new SatelliteTleDataResponse([]),
            'result' => new SatelliteTleDataResult([]),
        ...
        [$got, $err] = SatelliteTleDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SatelliteTleDataResult([])]);
        [, $err] = SatelliteTleDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SatelliteTleDataSpec([]), 'result' => null]);
        [, $err] = SatelliteTleDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SatelliteTleDataSpec(['step' => 's']),
            'result' => new SatelliteTleDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SatelliteTleDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SatelliteTleDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SatelliteTleDataSpec(['step' => 's']),
            'result' => new SatelliteTleDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SatelliteTleDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SatelliteTleDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SatelliteTleDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SatelliteTleDataSpec([]),
            'result' => new SatelliteTleDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SatelliteTleDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SatelliteTleDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SatelliteTleDataError('boom', 'boom');
        $utility->fetcher = function (SatelliteTleDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SatelliteTleDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SatelliteTleDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SatelliteTleDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SatelliteTleDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SatelliteTleDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        SatelliteTleDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SatelliteTleDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SatelliteTleDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SatelliteTleDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SatelliteTleDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SatelliteTleDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SatelliteTleDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SatelliteTleDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SatelliteTleDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SatelliteTleDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SatelliteTleDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SatelliteTleDataResult(['ok' => false])]);
        $this->expectException(SatelliteTleDataError::class);
        SatelliteTleDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SatelliteTleDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SatelliteTleDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        SatelliteTleDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SatelliteTleDataBaseFeature();
        $b = new SatelliteTleDataBaseFeature();
        SatelliteTleDataFeatureAdd::call($ctx, $a);
        SatelliteTleDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SatelliteTleDataBaseFeature
    {
        $f = new SatelliteTleDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SatelliteTleDataFeatureAdd::call($ctx, self::named_feature('a'));
        SatelliteTleDataFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        SatelliteTleDataFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        SatelliteTleDataFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        SatelliteTleDataFeatureAdd::call($ctx, $replace);
        ...
        SatelliteTleDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SatelliteTleDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SatelliteTleDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SatelliteTleDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        SatelliteTleDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SatelliteTleDataResponse(['headers' => null]),
            'result' => new SatelliteTleDataResult([]),
        ]);
        SatelliteTleDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        SatelliteTleDataResultBody::call($ctx);
