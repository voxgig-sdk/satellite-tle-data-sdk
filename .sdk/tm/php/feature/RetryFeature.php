...
// SatelliteTleData SDK retry feature
...
class SatelliteTleDataRetryFeature extends SatelliteTleDataBaseFeature
{
    ...
    public function init(SatelliteTleDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SatelliteTleDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SatelliteTleDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SatelliteTleDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
