...
[MCP](https://modelcontextprotocol.io) server exposing the SatelliteTleData SDK as
two agent tools — `satellite-tle-data_list` and `satellite-tle-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/satellite-tle-data-mcp)
make build

# 2. Provide credentials via the environment
export SATELLITE_TLE_DATA_APIKEY=sk_live_xxx
...
```jsonc
// satellite-tle-data_list: first page of records
{ "entity": "tle" }
{ "entity": "tle", "query": { } }

// satellite-tle-data_load: one record by id
{ "entity": "tle", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/satellite-tle-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user satellite-tle-data \
     -- "$PWD"/dist/*/satellite-tle-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `satellite-tle-data_list` and `satellite-tle-data_load` tools now appear
   in new sessions. Ask the agent to *"list tle using satellite-tle-data"*
   and it calls `satellite-tle-data_list` with `{"entity":"tle"}`.
...
```sh
export SATELLITE_TLE_DATA_APIKEY=sk_live_xxx            # API key
export SATELLITE_TLE_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `satellite-tle-data_list` tool
...
### Call the `satellite-tle-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `satellite-tle-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `satellite-tle-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SATELLITE_TLE_DATA_APIKEY` | API key sent with every request. |
| `SATELLITE_TLE_DATA_BASE` | Optional override of the API base URL. |
