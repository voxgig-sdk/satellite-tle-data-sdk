...
func TestTleEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Tle(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Tle(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		tleRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.tle")))
		...
		tleRef01DataDt0LoadResult := core.ToMapAny(entityData(tleRef01DataDt0Loaded))
...
func tleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SATELLITE_TLE_DATA_TEST_TLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SATELLITE_TLE_DATA_TEST_TLE_ENTID"])
