...
import (
	...
	omni "github.com/voxgig-sdk/satellite-tle-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SatelliteTleDataError); is {
				out := map[string]any{
					"name":    "SatelliteTleDataError",
...
func omniCtx(arg any, client *sdk.SatelliteTleDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SatelliteTleDataSDK); is {
			client = live
